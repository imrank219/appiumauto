...
public class ACCOUNT_SETTINGS_View_Policy extends BasicIOSTest {
    @Test
    public void ViewPolicyTest() throws IOException, InterruptedException {
        ...
        asp.viewpolicy.click();
        Thread.sleep(3000);

        Set<String> contextNames = driver2.getContextHandles();
        // driver2.context(contextNames.toArray()[1]);
        ...
        driver2.context((String) contextNames.toArray()[0]);

        System.out.println(((String)contextNames.toArray()[0]));

        String view = ((String)contextNames.toArray()[0]);
        if (view.contains("NATIVE_APP")) {
            asp.privacycenter.isDisplayed();
        }else if(view.contains("WEBVIEW")){
            driver2.findElement(By.xpath("//*[text()[contains(.,'Thank you for visiting the McGraw Hill Privacy Center.')]]")).isDisplayed();
        }
